...
import controlConfig from '../controls.json';
import {
  Avatar,
  Button,
  Card,
...
const steps = simplified
  ? ['Patient + urgency', 'Recommended slots', 'Confirmation']
  : [
      'Patient search',
      'Urgency details',
      'Appointment type',
...
const screens = simplified
  ? ['patient', 'slots', 'confirmation']
  : ['patient', 'urgency', 'type', 'provider', 'slots', 'operatory', 'confirmation'];
export default function Booking({ screen, state, controls }: PrototypeProps) {
  const editable =
    controls ?? Object.fromEntries(controlConfig.controls.map((c) => [c.id, c.value]));
  ...
  const slots = availableSlots(type).filter((s) => simplified || s.providerId === provider);
  const filtered = patients.filter((p) =>
    (p.name + ' ' + p.id + ' ' + p.phone).toLowerCase().includes(query.toLowerCase()),
  );
  const canContinue =
    active === 'patient'
      ? Boolean(patient) && (!simplified || reason.trim().length > 0)
      : active === 'urgency'
  ...
  return (
    <ClinicShell>
      ...
      <div className="nv-page-title">
        <div>
          <h1>Book an emergency appointment</h1>
          <p>
            {simplified
              ? 'A clear path from patient to appointment.'
                    ...
                    <>
                      ...
                      <details>
                        ...
                        <Field label="Internal note">
                          <Textarea placeholder="Optional scheduling notes" />
                        </Field>
                      </details>
                    </>
                  )}
              ...
              {active === 'slots' && (
                <>
                  <div className="nv-notice" data-playground-id="slot-guidance">
                    Same-day availability · {appointmentType.duration} minutes
                    <br />
                    <small>
                      Only compatible providers and rooms without a booking conflict are shown.
                    </small>
                  </div>
                  <div
                    className="nv-slot-grid"
                    data-playground-id="appointment-slots"
                    style={{ gap: editable['card-gap'] }}
                  >
                    {slots.slice(0, 12).map((s) => (
                      <button
                        ...
                        className="nv-choice"
                        style={{
                          padding: editable['card-padding'],
                          borderRadius: editable['card-radius'],
                          transitionProperty: 'background-color, border-color',
                          transitionDuration: editable['motion-duration'] + 'ms',
                        }}
                        aria-pressed={slot?.id === s.id}
                        onClick={() => setSlot(s)}
                      >
                        <strong style={{ fontSize: editable['title-size'] }}>{s.time}</strong>
              ...
              <div className="nv-actionbar">
                ...
                {active !== 'confirmation' && (
                  <Button disabled={!canContinue} onClick={() => setStep(step + 1)}>
                    {simplified && step === 0 ? 'Find available slots' : 'Continue'} →
                  </Button>
                )}
              </div>
            </>
