...
Project creation uses an inline non-modal form with required name, optional description, retained input on failure and exclusive file creation. No existing project is overwritten. User and NOVA navigation use separate URLs; dirty controls retain the existing departure guard. Prompt copy failures leave selectable text and inline recovery. Orientation dismissal uses a browser preference, while project identity stays in repository files.
